...
import { ARCH_UNITS } from '../data/architecturaLessons.js'
...
.ma-root{display:flex;flex-direction:column;height:100vh;height:100dvh;background:#0e0b07;color:#e8dfc8;font-family:'Crimson Pro',Georgia,serif;overflow:hidden;}
...
@media print {
  .sch-nav, .ma-sidebar, .ma-bnav, .ma-age-btns, .ma-lightbox { display: none !important; }
  .ma-root, .ma-body, .ma-content { height: auto !important; overflow: visible !important; }
  .ma-body { margin-top: 0 !important; }
  .ma-cols { display: block !important; padding: 0 !important; }
  .ma-root, .ma-root * { background: #fff !important; color: #000 !important; box-shadow: none !important; }
  .ma-card, .ma-kq, .ma-img-wrap { border: 1px solid #999 !important; break-inside: avoid; }
  .ma-lhdr { border-bottom: 2px solid #000 !important; }
  .ma-vchip { border: 1px solid #999 !important; }
  .ma-res-link { color: #000 !important; text-decoration: underline !important; }
  @page { margin: 0.6in; }
}
`
...
function readLastLesson() {
  try {
    const raw = localStorage.getItem('ma-last-lesson')
    const n = Number(raw)
    if (Number.isInteger(n) && n >= 0 && n < ALL.length) return n
  } catch (e) {}
  return 0
}

function Plate({ url, caption, alt, onOpen }) {
  const [errored, setErrored] = useState(false)
  return (
    <div className="ma-img-wrap">
      {errored ? (
        <div className="ma-img-cap" style={{ borderTop: 'none' }}>
          <div>{caption}</div>
          <div>Image unavailable — see Resources for the source.</div>
        </div>
      ) : (
        <>
          <img
            className="ma-img"
            src={`/img?url=${encodeURIComponent(url)}`}
            alt={alt}
            onClick={() => onOpen(url)}
            onError={() => setErrored(true)}
            title="Click to enlarge"
          />
          <div className="ma-img-cap">{caption} · Click to enlarge</div>
        </>
      )}
    </div>
  )
...
export default function MagisterArchitecturae() {
  const [ci, setCi] = useState(readLastLesson)
  ...
  useEffect(() => {
    try {
      localStorage.setItem('ma-last-lesson', String(ci))
    } catch (e) {}
  }, [ci])
              ...
              {lessons.map(lesson => {
                ...
                return (
                  <button key={lesson.id} className={`ma-lbtn${i === ci ? ' on' : ''}`} onClick={() => setCi(i)}>
                    <div className="ma-tp">{lesson.type}</div>
                    <div>{lesson.title.length > 30 ? lesson.title.slice(0, 30) + '…' : lesson.title}</div>
                  </button>
                )
              })}
            </div>
  ...
  return (
    <>
      ...
      {typeof lightbox === 'string' && (
        <div className="ma-lightbox" onClick={() => setLightbox(null)}>
          <img src={`/img?url=${encodeURIComponent(lightbox)}`} alt="" />
        </div>
      )}
      <div className="ma-root">
        <TopNav current="/magister-architecturae" />
        <div className="ma-body">
          ...
          <div className="ma-content" id="ma-content-scroll">
            ...
            <div className="ma-workbar">
              <span className="ma-wlbl">Source:</span>
              <span className="ma-wtag">{l.artist}</span>
              <span className="ma-wlbl" style={{ marginLeft: 8 }}>Subject:</span>
            ...
            <div className="ma-cols">
              <div className="ma-lcol">
                ...
                {/* Comparison badge */}
                {l.type === 'DESIGN' && (
                  <div className="ma-comp-badge">✦ Final Design</div>
                )}
                ...
                {l.kq && (
                  <div className="ma-kq">
                    <div className="ma-kqlbl">Key Question · Looking</div>
                ...
                {l.disc && l.disc.length > 0 && (
                  <div className="ma-card">
                    <div className="ma-chard" style={{ background: tc }}>Talk Together</div>
                ...
                {l.copywork && (
                  <div className="ma-card">
                    <div className="ma-chard" style={{ background: '#2a1a0a' }}>Copywork · Living Book</div>
                    <div className="ma-cbody">
                      <p style={{ fontStyle: 'italic' }}>{l.copywork}</p>
                      {l.copySource && <p style={{ marginTop: 8, fontSize: 12, color: '#9e8c72' }}>{l.copySource}</p>}
                    </div>
                  </div>
                )}
              ...
              <div className="ma-rcol">
                ...
                {l.materials && (
                  <div className="ma-card">
                    <div className="ma-chard" style={{ background: '#1a1508' }}>On the Table</div>
                    <div className="ma-cbody"><p>{l.materials}</p></div>
                  </div>
                )}
