...
//
// Lesson Viewer for Magister Historia — The American Founding, Unit 2.
// Reads HISTORIA_UNITS from src/data/historia.js.
//
// Route: /historia
// ─────────────────────────────────────────────────────────────────────────────
...
import { HISTORIA_UNITS } from "../data/historia.js";

export default function MagisterHistoria() {
  const [selectedId, setSelectedId] = useState("2.1");
      ...
      <header className="mh-topbar">
        <div className="mh-brand">
          ...
          <div>
            ...
            <span className="mh-brand-sub">— The American Founding</span>
          </div>
        </div>
        <div className="mh-controls">
          <div className="mh-form-toggle">
            {[
              { key: "both", label: "BOTH" },
              { key: "IA", label: "FORM IA" },
              { key: "IIA", label: "FORM IIA" },
            ].map(({ key, label }) => (
              <button
                key={key}
                className={form === key ? "mh-active" : ""}
                onClick={() => setForm(key)}
              >
                {label}
              </button>
            ...
            <div key={u.id} className={`mh-unit ${collapsed[u.id] ? "mh-collapsed" : ""}`}>
              ...
              <ul className="mh-lesson-list">
                {u.lessons.map((l) => (
                  <li
                    key={l.id}
                    className={`mh-lesson-item ${selectedId === l.id ? "mh-active" : ""}`}
        ...
        <main className="mh-main">
          {!found && (
            <div className="mh-empty">
              ...
              <p>
                Pick a lesson on the left. Both students hear the same stories —
                differentiation lives at the narration step. Use the form toggle
                to show only what each student needs.
              </p>
            </div>
          )}
          {found && (
            <Lesson unit={found.unit} l={found.lesson} form={form} typeClass={typeClass} />
          )}
        </main>
...
function Lesson({ unit, l, form, typeClass }) {
  ...
  return (
    <article className="mh-lesson">
      ...
      {/* Stories for the Heart — collapsible */}
      {l.storiesForHeart && l.storiesForHeart.length > 0 && (
        <>
          <h2
            className="mh-collapsible-head"
          ...
          {storiesOpen && (
            <ol className="mh-stories-list">
              {l.storiesForHeart.map((s, i) => (
                <li key={i}>{s}</li>
              ))}
            </ol>
          )}
          {!storiesOpen && (
            <div className="mh-collapsed-hint">
              {l.storiesForHeart.length} stories to tell — click to expand
            </div>
          )}
        </>
      )}
      ...
      {l.primarySource && (
        <div className="mh-primary-source">
          ...
          {l.primarySource.link && (
            <a href={l.primarySource.link} target="_blank" rel="noopener noreferrer">
              Read online ↗
            </a>
          )}
        </div>
      ...
      {l.persons && l.persons.length > 0 && (
        <>
          <h2>Persons</h2>
          <div className="mh-pill-row">
            {l.persons.map((p, i) => (
              <span key={i} className="mh-pill mh-pill-person">{p}</span>
            ))}
          </div>
        </>
      )}
      ...
      {l.geography && l.geography.length > 0 && (
        <>
          ...
          <div className="mh-pill-row">
            {l.geography.map((g, i) => (
              <span key={i} className="mh-pill mh-pill-geo">{g}</span>
            ))}
          </div>
        </>
      )}
      ...
      {l.vocab && l.vocab.length > 0 && (
        <>
          ...
          {vocabOpen && (
            <div className="mh-pill-row mh-pill-row-vocab">
              {l.vocab.map((v, i) => (
                <span key={i} className="mh-pill mh-pill-vocab">{v}</span>
              ))}
            </div>
          )}
          {!vocabOpen && (
            <div className="mh-collapsed-hint">
              {l.vocab.length} terms — click to expand
            </div>
          )}
        </>
      )}
      ...
      {l.discussion && l.discussion.length > 0 && (
        <>
          ...
          {discussOpen && (
            <ul className="mh-discussion-list">
              {l.discussion.map((q, i) => (
                <li key={i}>{q}</li>
              ))}
            </ul>
          )}
          {!discussOpen && (
            <div className="mh-collapsed-hint">
              {l.discussion.length} discussion questions — click to expand
            </div>
          )}
        </>
      )}
...
// ─────────────────────────────────────────────────────────────────────────────
// Styles — scoped to .mh-app, matching the existing app's design language
// ─────────────────────────────────────────────────────────────────────────────
const MH_STYLES = `
.mh-app {
  --mh-bg: #0a0906;
...
.mh-brand-title {
  ...
  color: var(--mh-parch); letter-spacing: 0.04em;
}
.mh-brand-sub {
  font-family: "IM Fell English", serif;
  font-style: italic;
  color: var(--mh-parch-dim); font-size: 14px; margin-left: 6px;
}
...
.mh-form-toggle button:last-child { border-right: none; }
.mh-form-toggle button:hover { background: var(--mh-bg-3); color: var(--mh-parch); }
.mh-form-toggle button.mh-active { background: var(--mh-gold); color: var(--mh-bg); }
.mh-btn {
  background: transparent; color: var(--mh-parch-dim);
  border: 1px solid var(--mh-rule-strong); border-radius: 4px;
...
/* ── Sidebar ─────────────────────────────────────────────────────── */
.mh-sidebar {
  background: var(--mh-bg-2); border-right: 1px solid var(--mh-rule);
  overflow-y: auto; padding: 10px 0 30px;
}
...
.mh-unit-title {
  ...
  color: var(--mh-gold-2); letter-spacing: 0.06em;
}
.mh-unit-sub {
  font-family: "IM Fell English", serif; font-style: italic;
  font-size: 12px; color: var(--mh-parch-mute); margin-top: 2px;
}
...
.mh-lesson-item {
  ...
  padding: 7px 20px 7px 30px; cursor: pointer;
  color: var(--mh-parch-dim); font-size: 14px;
  border-left: 2px solid transparent;
}
.mh-lesson-item:hover { background: var(--mh-bg-3); color: var(--mh-parch); }
.mh-lesson-item.mh-active {
  background: linear-gradient(to right, rgba(192,138,32,0.18), transparent);
  border-left-color: var(--mh-gold); color: var(--mh-parch);
}
.mh-lesson-id {
  font-family: "Cinzel", serif; font-size: 11px;
  color: var(--mh-gold); letter-spacing: 0.08em; min-width: 28px;
}
.mh-lesson-type {
...
.mh-type-STORY { color: var(--mh-story); border-color: var(--mh-story); }
.mh-type-SEMINAR { color: var(--mh-seminar); border-color: var(--mh-seminar); }
...
.mh-lesson h1 {
  ...
  color: var(--mh-parch); margin: 0 0 4px; line-height: 1.15; letter-spacing: 0.02em;
}
.mh-dates {
  font-family: "Cinzel", serif; font-size: 12px; color: var(--mh-gold);
  letter-spacing: 0.2em; text-transform: uppercase; margin-bottom: 8px;
}
.mh-tagline {
  font-family: "IM Fell English", serif; font-style: italic;
  color: var(--mh-gold-2); font-size: 18px; margin-bottom: 28px;
  border-bottom: 1px solid var(--mh-rule); padding-bottom: 22px;
}

.mh-lesson h2 {
  font-family: "Cinzel", serif; font-size: 13px; color: var(--mh-gold);
  letter-spacing: 0.18em; text-transform: uppercase;
  margin: 36px 0 12px; padding-bottom: 6px;
  border-bottom: 1px solid var(--mh-rule);
}
.mh-label {
  font-family: "Cinzel", serif; font-size: 10px; color: var(--mh-gold);
...
.mh-toggle-icon {
  font-size: 10px; color: var(--mh-parch-mute);
  transition: transform 0.2s; margin-left: 8px;
}
.mh-toggle-icon.mh-open { transform: rotate(180deg); }
.mh-collapsed-hint {
  font-family: "IM Fell English", serif; font-style: italic;
  color: var(--mh-parch-mute); font-size: 14px; margin: 4px 0 20px;
  cursor: pointer;
}

/* Blocks */
.mh-objective {
  background: rgba(192,138,32,0.06); border-left: 3px solid var(--mh-gold);
  padding: 12px 16px; margin: 18px 0; color: var(--mh-parch-dim); font-size: 15px;
}
.mh-teacher-prep {
  background: rgba(192,138,32,0.08); border-left: 3px solid var(--mh-gold);
  padding: 12px 16px; margin: 18px 0; font-size: 14px; color: var(--mh-parch-dim);
}
.mh-opening {
  font-size: 18px; color: var(--mh-parch); line-height: 1.6;
  padding-left: 20px; border-left: 3px solid var(--mh-gold);
  margin-bottom: 26px;
}

.mh-stories-list {
  margin: 8px 0 18px; padding-left: 24px;
}
.mh-stories-list li {
  margin-bottom: 10px; color: var(--mh-parch); line-height: 1.5;
}

.mh-spine-block {
  background: linear-gradient(135deg, var(--mh-bg-2), var(--mh-bg-3));
  border: 1px solid var(--mh-gold); border-radius: 4px;
...
.mh-spine-title {
  ...
  color: var(--mh-parch); font-style: italic; line-height: 1.35;
}
.mh-spine-pages { font-size: 13px; color: var(--mh-parch-mute); margin-top: 4px; }
.mh-spine-link { margin-top: 6px; font-size: 13px; }

.mh-primary-source {
  background: rgba(94,122,160,0.08); border: 1px solid rgba(94,122,160,0.4);
  border-radius: 4px; padding: 14px 18px; margin: 18px 0;
}
.mh-primary-source .mh-label { color: var(--mh-blue); }
.mh-ps-title {
  font-family: "IM Fell English", serif; font-style: italic;
  font-size: 16px; color: var(--mh-parch); margin-bottom: 6px;
}

.mh-age-block {
  background: var(--mh-bg-2); border-left: 3px solid;
  padding: 14px 18px; margin: 12px 0; border-radius: 0 3px 3px 0;
}
...
.mh-copywork {
  font-family: "IM Fell English", serif; font-style: italic;
  font-size: 16px; color: var(--mh-parch); line-height: 1.6;
  margin: 10px 0 0; padding: 0;
  border: none;
}

.mh-memory {
  background: rgba(192,138,32,0.06); border: 1px solid rgba(192,138,32,0.3);
  border-radius: 4px; padding: 16px 20px; margin: 18px 0;
}
.mh-memory-text {
  font-family: "IM Fell English", serif; font-style: italic;
  font-size: 15px; color: var(--mh-parch); line-height: 1.7;
  white-space: pre-wrap; margin: 0;
}

.mh-notebooking {
  background: rgba(90,140,88,0.07); border: 1px solid rgba(90,140,88,0.35);
  border-radius: 4px; padding: 14px 18px; margin: 18px 0;
}
...
.mh-pill {
  padding: 4px 12px; border-radius: 2px; font-size: 13px;
  font-family: "IM Fell English", serif; letter-spacing: 0.02em;
}
.mh-pill-person {
  background: rgba(192,138,32,0.08); border: 1px solid rgba(192,138,32,0.35);
  color: var(--mh-parch-dim);
}
.mh-pill-geo {
  background: rgba(94,122,160,0.08); border: 1px solid rgba(94,122,160,0.35);
  color: var(--mh-parch-dim);
}
.mh-pill-vocab {
  background: var(--mh-bg-3); border: 1px solid var(--mh-rule-strong);
  color: var(--mh-parch-mute);
}

.mh-discussion-list {
  margin: 8px 0 18px; padding-left: 22px;
}
.mh-discussion-list li { margin-bottom: 9px; color: var(--mh-parch); }

.mh-civics {
  background: var(--mh-bg-2); border: 1px solid var(--mh-rule);
  border-radius: 4px; padding: 14px 18px; margin: 18px 0; font-size: 13px;
}
.mh-civics ul { margin: 6px 0 0; padding-left: 20px; }
.mh-civics li { color: var(--mh-parch-mute); margin-bottom: 4px; }

.mh-watchfor {
  background: rgba(160,80,72,0.07); border: 1px solid rgba(160,80,72,0.35);
  border-radius: 4px; padding: 14px 18px; margin: 18px 0; color: var(--mh-parch-dim);
}
...
.mh-empty {
  ...
  height: 100%; color: var(--mh-parch-mute);
  text-align: center; padding: 40px;
}
.mh-quill { font-size: 48px; color: var(--mh-gold); margin-bottom: 18px; opacity: 0.6; }
.mh-empty h2 {
  color: var(--mh-gold-2); border: none; margin: 0 0 8px;
  font-family: "Cinzel", serif; font-size: 14px; letter-spacing: 0.18em;
  text-transform: uppercase;
}
.mh-empty p {
  font-style: italic; color: var(--mh-parch-dim); max-width: 480px;
}
...
@media print {
  ...
  .mh-topbar, .mh-sidebar, .mh-form-toggle, .mh-btn { display: none !important; }
  ...
  .mh-spine-title, .mh-ps-title { color: #1a1a1a; font-style: italic; }
  ...
  .mh-stories-list, .mh-discussion-list, .mh-pill-row-vocab { display: block !important; }
  @page { margin: 0.7in; }
}
`;
